...
class WAFTv2(nn.Module):
    ...
    def forward(self, image1, image2, iters=None, flow_gt=None, return_aux=False):
        ...
        for itr in range(iters):
            ...
            trajectory_low.append(flow_2x)
            updates_low.append(flow_update[:, :2])
        ...
        aux = None
        if return_aux:
            top, bottom, left, right = self._half_unpad_slices(padder)
            aux = {
                "fmap1": fmap1_2x[..., top:bottom, left:right],
                "fmap2": fmap2_2x[..., top:bottom, left:right],
                "hidden": net[..., top:bottom, left:right],
                "flow_low": flow_2x[..., top:bottom, left:right],
                "last_update": flow_update[:, :2, top:bottom, left:right],
                "trajectory_low": [flow[..., top:bottom, left:right] for flow in trajectory_low],
                "updates_low": [update[..., top:bottom, left:right] for update in updates_low],
            }
    ...
    @staticmethod
    def _half_unpad_slices(padder):
        """Return the original-image crop for H/2 tensors."""
        left, right, top, bottom = padder._pad
        if any(value % 2 for value in (left, right, top, bottom)):
            raise ValueError("WAFT auxiliary feature padding must be divisible by two")
        return top // 2, -bottom // 2 or None, left // 2, -right // 2 or None
